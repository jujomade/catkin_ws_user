...
def callbackBackwardLongitudinal(request):
    ...
    driving_direction_pub1.publish(0.20)

    #rospy.sleep(5)
    #pub_back.publish(0.08)

    rospy.sleep(5)
    driving_direction_pub2.publish(0.20)

    rospy.sleep(5)
    pub_forward.publish(0.07)
...
# run simultaneously.
rospy.init_node("simple_parking_maneuver", anonymous=True)
